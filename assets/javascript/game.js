...
function main() {
	...
	$('#winscolumn').html('<h3><em>Wins: </em><span>0</span></h3>');
	$('#lossescolumn').html('<h3><em>Losses: </em><span>0</span></h3>');
	...
	$('#attack').on('click', function() {
		...
		$('#gameupdates').append('<span>You attacked the defender for ' + defenderTotalAttack + ' hitpoints!</span>');
		if (attackerhitpoints <= 0) {
			losses++;
			$('#attack').prop('disabled', true);
			$('#gameupdates').html('<h3><em>Game Updates: </em></h3><span>You\'ve lost!</span>');
			$('#lossescolumn').html('<h3><em>Losses: </em><span>' + losses + '</span></h3>');
		}
			...
			if (defenderLosses == people.length - 1) {
				...
				$('#winscolumn').html('<h3><em>Wins: </em><span>' + wins + '</span></h3>');
				cowabunga.play();
				return;
			}
			$('#attack').prop('disabled', true);
